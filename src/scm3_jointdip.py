## JointDIP on SCM 3 with different lambdas
...
for seed in seeds:
    ...
    for lamJointDIP in lambdas:
        ...
        mjointDIP = src.method.CIP_JointCIPDIP(device=device, model=model, lamDIP=lamJointDIP, 
                                                  ...
                                                  nb_classes=2, loss_type='CrossEntropyLoss', optimizer='Adam', lr=1e-2)
        mjointDIP.fit(dataloaders, None, [0], M-1, epochs=50, verbose_every=51)
        ...
        save_file = f'../results/DIPfail_SCM3_seed_{seed}_JointDIP_{lamJointDIP}_1.0_.npy'
